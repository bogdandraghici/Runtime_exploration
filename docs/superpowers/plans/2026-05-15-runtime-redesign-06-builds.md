...
**Goal:** Specify the Builds page with the new deployment lens (coverage column on rows + Deployment tab on detail). Resolves friction #6.

**Architecture:** Branch tabs as primary filter; rows show coverage bar + live stats (runs/errors/quality with deltas); detail panel adds Deployment, Comparison, Live runs, Workflows, Changelog tabs.

**Deliverables:**
- Create: `docs/superpowers/specs/pages/builds.md`
- Create: Figma frames "Builds / List", "… / Detail — Deployment tab", "… / Comparison tab"

**Depends on:** Plan 01, Plan 02. Coordinates with Plan 08 (Live Evals comparison endpoint).
...
- [ ] **Step 2: Commit**: `spec: builds information model`
...
- [ ] **Step 2: Commit**: `spec: builds component API`
...
Figma frame "Builds / List" with branch tabs (All · main · staging · beta), 4 rows (one selected active build with coverage 78%, one minor traffic on legacy role 15%, one beta 7%, one archived 0%). Live stats with deltas.

- [ ] **Step 2: Detail — Deployment tab**

Frame: "Who it's serving" rows (default policy + 2 overrides), "Not on this build" footer line.

- [ ] **Step 3: Detail — Comparison tab**

Frame: cmp-grid with 3 cells (throughput / p95 / error rate) showing deltas, plus scorer deltas below.

- [ ] **Step 4: Commit**: `spec(builds): list + 2 detail tab mockups`
...
- [ ] **Step 3: Commit**: `spec(builds): interaction states`
...
- [ ] **Step 2: Commit**: `spec(builds): copy`
...
- [ ] **Step 2: Commit**: `spec(builds): design tokens`
...
5 tasks: (a) tell me which build is currently serving everyone, (b) which build is serving the legacy-tier role, (c) compare v1.4.2 to v1.4.1 and tell me which got worse, (d) find a live run on v1.4.2 and open it, (e) explain what "coverage" means here.

- [ ] **Step 2: Run with 5 participants (emphasize developer + ops mix)**

- [ ] **Step 3: `## User test summary`**

- [ ] **Step 4: Commit**: `spec(builds): user-test summary`
...
Items: coverage computation strategy (sampling vs exact), refresh frequency, Comparison data source (reuse Live Evals BuildComparison entity?), Rollback API contract, archive policy (when is a build archived?).

- [ ] **Step 3: Mark "Ready for engineering"**

- [ ] **Step 4: Commit**: `spec(builds): handoff`
